...
void hough_cpu(matrix &m, std::vector<float> vettore_di_bin, std::vector<float> vec_x, std::vector<float> vec_y)
{
...
void ricerca_max_cpu(int array_a[num_bins_a][num_bins_b], std::vector<float> vettore_di_bin, std::vector<float> vettore_coeff_ang, int threshold , unsigned int event_ID, std::ofstream &reco_retta)
{
	int array_max[num_bins_a];
	int loc[num_bins_a];
	for(int i=0; i<num_bins_a; i++)
	{
		int location = 1;
	...
	for(int i=0; i<num_bins_a; i++)
	{
		if(i>0 and i<num_bins_a and array_max[i] > threshold and array_max[i-1] < array_max[i] and array_max[i] > array_max[i+1])
		{
			reco_retta << "Evento " << event_ID << "\tMassimo " << array_max[i-1] << "\tCoeffAng " << vettore_coeff_ang[i-1] << "\toffset " << vettore_di_bin[loc[i-1]]  << std::endl;
		}
		if(i>2 and i<num_bins_a-1 and array_max[i]>threshold and array_max[i-1] == array_max[i] and array_max[i-1]>array_max[i-2] and array_max[i]>array_max[i+1])
		{
			reco_retta << "Evento " << event_ID << "\tMassimo " << array_max[i-1] << "\tCoeffAng " << vettore_coeff_ang[i-1] << "\toffset " << vettore_di_bin[loc[i-1]]  << std::endl;				
		}
	}
...
void ricerca_max_cpu_locale(int array_a[num_bins_a][num_bins_b], std::vector<float> vettore_di_bin, std::vector<float> vettore_coeff_ang,unsigned int event_ID, int threshold, std::ofstream &reco_retta)
{
	int array_max[num_bins_a];
	int loc[num_bins_a];
	...
	for(int bin_a=0; bin_a<num_bins_a; bin_a++)
	{
		...
		for(int c=0; c<num_bins_b; c++)
		{
			if(array_a[bin_a][c]>threshold and c>2 and c<num_bins_b-2 and array_a[bin_a][c]>=array_a[bin_a][c-1] and array_a[bin_a][c]>=array_a[bin_a][c-2] and array_a[bin_a][c]>=array_a[bin_a][c+1] and array_a[bin_a][c]>=array_a[bin_a][c+2])
				if(bin_a>2 and bin_a<num_bins_a-2 and array_a[bin_a][c]>array_a[bin_a-1][c] and array_a[bin_a][c]>array_a[bin_a-2][c] and array_a[bin_a][c]>array_a[bin_a+1][c] and array_a[bin_a][c]>array_a[bin_a+2][c])					
					if(array_a[bin_a][c]>array_a[bin_a-1][c-1] and array_a[bin_a][c]>array_a[bin_a-1][c+1] and array_a[bin_a][c]>array_a[bin_a+1][c-1] and array_a[bin_a][c]>array_a[bin_a+1][c+1]){
						//if(array_a[bin_a][c]>histo[(bin_a-2)*num_bins_b+(c-1)] and histo[bin_a*num_bins_a+c]>histo[(bin_a-2)*num_bins_b+(c+1)] and array_a[bin_a][c]>histo[(bin_a+2)*num_bins_b+(c-1)] and array_a[bin_a][c]>histo[(bin_a+2)*num_bins_b+(c+1)])
							//if(array_a[bin_a][c]>histo[(bin_a-1)*num_bins_b+(c-2)] and array_a[bin_a][c]>histo[(bin_a-1)*num_bins_b+(c+2)] and array_a[bin_a][c]>histo[(bin_a+1)*num_bins_b+(c-2)] and array_a[bin_a][c]>histo[(bin_a+1)*num_bins_b+(c+2)])				
								//fprintf(reco_retta, "Evento %d\tMassimo %d\tCoeffAng %.3f \toffset %.3f \n",event_ID, array_a[bin_a][c], vettore_coeff_ang[bin_a], vettore_di_bin[c]);	
								reco_retta << "Evento " << event_ID << "\tMassimo " << array_a[bin_a][c] << "\tCoeffAng" << vettore_coeff_ang[bin_a] << "\toffset " << vettore_di_bin[c] << std::endl;			
						}
...
int main()
{
	...
	full_header(data);
	//file su cui stampo le coefficiente angolare e offset rette ricostruite
	...
	//float vettore_di_bin[num_bins_b];
...
//		std::cout  << vettore_di_bin[i] << " " ;
	...
	while(!data.eof())
	{
		...
		// SUL SECONDO ASSE INSERISCO I COEFFICIENTI ANGOLARI TROVATI
			...
			unsigned int event_ID = vec_eventi[1].event_ID();
			...
			std::cout << "Time for hough matrix filling execution: " << t_hough <<" us" <<  std::endl;
			...
			std::cout << "Time for histogram filling: " << t_histo << " ms" << std::endl;


			std::cout << "Ora parte ricerca massimi: " << std::endl;
			auto start_max = std::chrono::steady_clock::now();
			//ricerca_max_cpu(array_a, vettore_di_bin, vettore_coeff_ang, threshold, event_ID, reco_retta);
			ricerca_max_cpu_locale(array_a, vettore_di_bin, vettore_coeff_ang, event_ID, threshold, reco_retta);
			...
			std::cout << "Time to find local max: " << t_max << " us" << std::endl;
